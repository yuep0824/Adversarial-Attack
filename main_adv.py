...
from attacks.PGD import pgd_attack, multi_restart_pgd_attack, nes_pgd_attack, cw_pgd_hybrid_attack, pgd_attack_with_mask, mask_block_attack
...
if __name__ == '__main__':
    adv_attack = 'mask_block'  # 可选：fgsm, pc_i_fgsm, pgd, deepfool, cw, boundary, nes_pgd_attack, multi_restart_pgd, cw_pgd_hybrid_attack， mask_pgd, mask_block
    ...
    # model_list = ['cifar10_resnet56', 'cifar10_vgg16_bn', 'cifar10_mobilenetv2_x1_0', 'cifar10_shufflenetv2_x2_0', 'densenet121', 'densenet161', 'densenet169', 'googlenet', 'inception_v3', 'mobilenet_v2', 'vgg11_bn', 'vgg13_bn', 'vgg16_bn', 'vgg19_bn']
    for attack_model in model_list:
        ...
        for image_name in tqdm(os.listdir(clean_image_folder), desc="Adversarial Processing"):
            ...
            if adv_attack == 'fgsm':
                ...
                adversarial_tensor = pgd_attack(model, input, label, epsilon=1.5, alpha=0.1, num_iterations=30, device=device)
            elif adv_attack == 'mask_pgd':
                adversarial_tensor = pgd_attack_with_mask(model, input, label, epsilon=1.5, alpha=0.1, num_iterations=30, n_blocks=8, device=device)
            elif adv_attack == 'mask_block':
                adversarial_tensor = mask_block_attack(model, input, label, n_blocks=6)
                ...
                adversarial_tensor = nes_pgd_attack(model, input, label, epsilon=0.4, alpha=0.05, num_iterations=20)
